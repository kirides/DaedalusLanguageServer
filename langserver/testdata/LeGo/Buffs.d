...
func void _Buff_Dispatcher(var int bh) { // This is called every tick and is responsible for deleting the object 
		if (!Hlp_IsValidHandle(bh)) {
			// TODO: (Kirides) - Gehört das hier nicht gelöscht?
			MEM_Warn("Buffhandle invalid, removing accompanying FrameFunction");
...
// FIXME: Workaround für Ninja <= 2610, damit die toten FrameFunctions aus dem Save verschwinden
func void _Buff_Dispatcher_Workaround(var int bh) { // This is called every tick and is responsible for deleting the object 
		if (!Hlp_IsValidHandle(bh)) {
			// TODO: (Kirides) - Gehört das hier nicht gelöscht?
			MEM_Warn("Buffhandle invalid, removing accompanying FrameFunction");
			FF_RemoveData(_Buff_Dispatcher_Workaround, bh);
			return;
		};
...
// FIXME: Workaround für Ninja <= 2610, damit die toten FrameFunctions aus dem Save verschwinden
func int _Buff_Apply_Workaround(var c_npc npc, var int buff) {
		var int bh; bh = new(buff);
		var lCBuff b; b = get(bh);
		...
		FF_ApplyExtDataGT(_Buff_Dispatcher_Workaround, b.tickMS, -1, bh);
